"""adtx CLI: run an attribution case, or manage the corpus index."""
...
def _run(args: argparse.Namespace) -> int:
    ...
    ua = (f"adtx-attribution/0.1 (case {scope.case_ref}; "
...
def _portfolio(args: argparse.Namespace) -> int:
    ...
    fetcher = Fetcher(
        user_agent="adtx-attribution/0.2 (+https://github.com/OWNER/adtx-attribution)",
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="adtx", description="Ad-tech attribution")
    sub = ap.add_subparsers(dest="cmd", required=True)

    r = sub.add_parser("run")
    r.add_argument("--case", required=True)
    r.add_argument("--out", default="./out")
    r.add_argument("--index", default="", help="corpus sqlite from `adtx-index build`")
...
def _cli() -> int:
    """Entry point tolerant of a truncated pipe (`adtx registries | head`)."""
    import contextlib
    import os
    try:
        return main()
